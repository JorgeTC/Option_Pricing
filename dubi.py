...
def check_parameter(option):
    if option == "S0":
        K = request_parameter("Enter the Strike Price: ", 0, float('inf'), is_integer = False)
        r = request_parameter("Enter the risk-free rate: ", 0, float(1), is_integer = False)
        T = request_parameter("Enter the Time to Maturity: ", 0, float('inf'), is_integer = False)
        N = request_parameter("Enter the Number of steps: ", 2, float(1000), is_integer = True)
        return {"K":K, "r":r, "T":T, "N":N}
    elif option == "K":
        S0 = request_parameter("Enter the Spot Price: ", 0, float('inf'), is_integer = False)
        r = request_parameter("Enter the risk-free rate: ", 0, float(1), is_integer = False)
        T = request_parameter("Enter the Time to Maturity: ", 0, float('inf'), is_integer = False)
        N = request_parameter("Enter the Number of steps: ", 2, float(1000), is_integer = True)
...
param_to_graph = request_parameter_to_graph("Enter a parameter of the list:", valid_list)
constant_parameters = check_parameter(param_to_graph)
constant_values = {**constant_parameters, "params" : {"sigma": 0.3, "is_call": True, "is_eu": True}}
